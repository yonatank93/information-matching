...
def test_array():
    """Test the preconditioning function if an array is given."""
    J = np.random.randn(npreds, nparams)
    I = J.T @ J
    # Reference value
    ref = {"fim": I, "fim_scale": 1 / np.linalg.norm(I)}
    # Use preconditioning
    test_val = preconditioning(I, "frobenius")

    assert ref == test_val


def test_dict():
    """Test the preconditioning function if a dictionary is given."""
    ...
    # Use preconditioning
    test_val = preconditioning(dict1, "frobenius")
    ...
    # Use preconditioning
    test_val = preconditioning(dict2, "frobenius")
    assert ref1 == test_val

    # Test using "max_frobenius" scaling type
    test_val = preconditioning(dict1, "max_frobenius")
...
def test_exception():
    ...
    with pytest.raises(ValueError):
        ...
        preconditioning(dict2, "frobenius")
    with pytest.raises(ValueError):
