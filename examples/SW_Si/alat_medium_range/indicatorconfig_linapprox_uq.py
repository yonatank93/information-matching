...
dalist = np.arange(-1.0, 2.01, 0.05)
...
model_target = ModelEnergyCurve(
    data=energycurve_data["data"], error=target_error, da_list=dalist
)
# Data and target error bars
elist = np.insert(model_target.data, 20, 0.0)
eng_error = np.insert(model_target.std, 20, 0.0)
...
# mean_target_full = np.insert(np.mean(samples_target_full, axis=0), 20, 0.0)
# error_target_full = np.insert(np.std(samples_target_full, axis=0), 20, 0.0)
...
# )
# mean_target_lin = np.insert(np.mean(samples_target_lin, axis=0), 20, 0.0)
# error_target_lin = np.insert(np.std(samples_target_lin, axis=0), 20, 0.0)
# Closed-form expression
mean_target_lin = np.insert(f0, 20, 0.0)
cov_target_lin = J_target @ cov @ J_target.T
error_target_lin = np.insert(np.sqrt(np.diag(cov_target_lin)), 20, 0.0)
