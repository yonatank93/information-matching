...
from model_target import ModelLatticeElastics


mpl.use("qt5agg")
np.random.seed(2022)

# This directory, i.e., the woking directory
WORK_DIR = Path(__file__).absolute().parent
...
elasticconstants_data = np.load("elasticconstants_data.npz")
target_data = elasticconstants_data["data"]
target_error = elasticconstants_data["error"]
model_target = ModelLatticeElastics(data=target_data, error=target_error)
...
# mean_target_full = np.mean(samples_target_full, axis=0)
# error_target_full = np.std(samples_target_full, axis=0)
# print("Mean:", mean_target_full)
# print("Error:", error_target_full)
...
# )
# mean_target_lin = np.mean(samples_target_lin, axis=0)
# error_target_lin = np.std(samples_target_lin, axis=0)
# Closed-form expression
mean_target_lin = f0
cov_target_lin = J_target @ cov @ J_target.T
error_target_lin = np.sqrt(np.diag(cov_target_lin))

print("Mean:", mean_target_lin)
print("Error:", error_target_lin)
print("Target error:", target_error)
